...
export default class WebPersonalWork extends React.Component {
    ...
    const base = ()=> {
      return (
        <tr>
          ...
          <td className={styles.value}>{personal.cjhzpx}</td>

          <td className={styles.label}>培训需求</td>
          <td className={styles.value}>{personal.pxxj}</td>
        </tr>
      )
    }
    ...
    const body1 = ()=> { //外出务工
      return (
        ...
        <tr>
          ...
          <td className={styles.value} colSpan={3}>{personal.wgdd}</td>
        </tr>
        {base()}
        </tbody>
      )
