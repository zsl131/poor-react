    ...
    const SingleCount = (dto) => {
      return (
        <p className={styles.singleCount}>
          ...
          <span className={styles.label}>转移就业人数：</span><b className={styles.value}>{dto.jys}</b>，
          <span className={styles.label}>转移就业人数占比：</span><b className={styles.value}>{dto.jybl + "%"}</b>
        </p>
      )
    };
